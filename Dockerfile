...
#RUN mkdir -p /var/php-fpm/log/
#
#RUN chmod -R 755 /var/php-fpm/
#RUN chown -R www-data:www-data /var/php-fpm/
USER root
RUN chmod -R 755 /usr/local/var/log/
...
# Шаги установки и настройки PHP

# Шаги для библиотеки
USER root
RUN mkdir -p /usr/local/lib/php/eusphpe_extension

RUN chmod -R 755 /usr/local/lib/php/eusphpe_extension
RUN chown -R www-data:www-data /usr/local/lib/php/eusphpe_extension

COPY /docker/lib/eusphpei.tar /app/eusphpei.tar

RUN tar -xC /usr/local/lib/php/eusphpe_extension -f /app/eusphpei.tar

# Экспорт LD_LIBRARY_PATH
RUN echo 'export LD_LIBRARY_PATH=/usr/local/php/eusphpe_extension' >> /usr/local/etc/init.d/php-fpm

# Создание файла конфигурации для библиотеки
RUN echo 'extension="/usr/local/lib/php/eusphpe_extension/eusphpe.so"' > /usr/local/etc/php/conf.d/eusphpe.ini
